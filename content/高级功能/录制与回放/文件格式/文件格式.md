...
CARLA记录器系统将重放模拟所需的所有信息保存在一个二进制文件中，该文件采用小端字节序存储多字节值。文件格式包含一个小型头部，其中包含版本、魔数字符串、日期和所使用的地图等通用信息，以及多种类型的数据包（目前使用10种类型，未来可能会继续增加）。

**Section sources**
- [ref_recorder_binary_file_format.md](https://github.com/carla-simulator/carla/blob/ue5-dev/Docs/ref_recorder_binary_file_format.md#L3-L29)
...
**Section sources**
- [ref_recorder_binary_file_format.md](https://github.com/carla-simulator/carla/blob/ue5-dev/Docs/ref_recorder_binary_file_format.md#L34-L40)

## 头部信息

头部信息包含关于记录文件的一般信息。它包含版本号和用于标识文件为记录器文件的魔数字符串。如果头部发生变化，版本号也会随之改变。此外，它还包含一个时间戳（从1900年纪元开始的秒数）和一个字符串，表示用于记录的地图名称。
...
**Section sources**
- [ref_recorder_binary_file_format.md](https://github.com/carla-simulator/carla/blob/ue5-dev/Docs/ref_recorder_binary_file_format.md#L43-L55)
...
每个数据包以一个5字节的小头部开始，包含两个字段：
- **id**：数据包类型
- **size**：数据包数据的大小

头部信息后跟的是**data**。**data**是可选的，大小为0表示数据包中没有**data**。如果**size**大于0，则表示数据包有**data**字节。因此，需要根据数据包的类型重新解释**data**。
...
**Diagram sources**
- [ref_recorder_binary_file_format.md](https://github.com/carla-simulator/carla/blob/ue5-dev/Docs/ref_recorder_binary_file_format.md#L59-L64)

**Section sources**
- [ref_recorder_binary_file_format.md](https://github.com/carla-simulator/carla/blob/ue5-dev/Docs/ref_recorder_binary_file_format.md#L57-L75)
...
**Diagram sources**
- [ref_recorder_binary_file_format.md](https://github.com/carla-simulator/carla/blob/ue5-dev/Docs/ref_recorder_binary_file_format.md#L217-L222)

**Section sources**
- [ref_recorder_binary_file_format.md](https://github.com/carla-simulator/carla/blob/ue5-dev/Docs/ref_recorder_binary_file_format.md#L215-L229)

## 核心数据结构

### CarlaRecorderFrame

该结构表示一个帧，包含帧ID、当前帧的持续时间和经过的时间。
...
**Diagram sources**
- [CarlaRecorderFrames.h](https://github.com/carla-simulator/carla/blob/ue5-dev/Unreal/CarlaUnreal/Plugins/Carla/Source/Carla/Recorder/CarlaRecorderFrames.h#L12-L21)

### CarlaRecorderEventAdd

该结构表示添加参与者的事件，包含数据库ID、类型、位置、旋转和参与者描述。
...
**Diagram sources**
- [CarlaRecorderEventAdd.h](https://github.com/carla-simulator/carla/blob/ue5-dev/Unreal/CarlaUnreal/Plugins/Carla/Source/Carla/Recorder/CarlaRecorderEventAdd.h#L26-L32)
...
**Diagram sources**
- [CarlaRecorderPosition.h](https://github.com/carla-simulator/carla/blob/ue5-dev/Unreal/CarlaUnreal/Plugins/Carla/Source/Carla/Recorder/CarlaRecorderPosition.h#L13-L17)

### CarlaRecorderStateTrafficLight

该结构记录交通灯的状态，包括ID、是否冻结、经过时间和状态。
...
**Diagram sources**
- [CarlaRecorderState.h](https://github.com/carla-simulator/carla/blob/ue5-dev/Unreal/CarlaUnreal/Plugins/Carla/Source/Carla/Recorder/CarlaRecorderState.h#L13-L18)
...
此数据包说明在当前帧需要创建多少参与者。字段**total**表示后续记录的数量。每个记录以**id**字段开始，即参与者在记录时具有的ID。**type**可以有以下可能值：
- 0 = 其他
- 1 = 车辆
- 2 = 行人
- 3 = 交通灯
...
此数据包说明哪个参与者是另一个（父级）的子级。第一个id是子参与者，第二个是父参与者。
...
**Section sources**
- [ref_recorder_binary_file_format.md](https://github.com/carla-simulator/carla/blob/ue5-dev/Docs/ref_recorder_binary_file_format.md#L84-L194)
...
**Section sources**
- [CarlaRecorder.cpp](https://github.com/carla-simulator/carla/blob/ue5-dev/Unreal/CarlaUnreal/Plugins/Carla/Source/Carla/Recorder/CarlaRecorder.cpp#L446-L453)
- [CarlaRecorderHelpers.cpp](https://github.com/carla-simulator/carla/blob/ue5-dev/Unreal/CarlaUnreal/Plugins/Carla/Source/Carla/Recorder/CarlaRecorderHelpers.cpp#L88-L104)

## 版本兼容性与升级

当头部信息发生变化时，版本号也会改变。建议使用100以上的**id**作为用户自定义数据包，因为此列表将在未来继续增长。

**Section sources**
- [ref_recorder_binary_file_format.md](https://github.com/carla-simulator/carla/blob/ue5-dev/Docs/ref_recorder_binary_file_format.md#L46-L47)
- [CarlaRecorder.h](https://github.com/carla-simulator/carla/blob/ue5-dev/Unreal/CarlaUnreal/Plugins/Carla/Source/Carla/Recorder/CarlaRecorder.h#L48-L73)
...
**Section sources**
- [CarlaRecorder.cpp](https://github.com/carla-simulator/carla/blob/ue5-dev/Unreal/CarlaUnreal/Plugins/Carla/Source/Carla/Recorder/CarlaRecorder.cpp#L532-L541)
- [CarlaRecorder.h](https://github.com/carla-simulator/carla/blob/ue5-dev/Unreal/CarlaUnreal/Plugins/Carla/Source/Carla/Recorder/CarlaRecorder.h#L184-L185)